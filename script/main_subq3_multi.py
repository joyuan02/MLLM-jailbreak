...
3. Third sub-question"""
    ...
    # Fallback: split sentences or chunk
    sents = re.split(r'[。\.!?？！\n]+', prompt_text)
...
def split_24_for_three(selected_list: List[str], need_each: int = 8) -> Tuple[List[str], List[str], List[str]]:
    """
    Prefer 24 unique images -> split into 3 groups of 8.
    If fewer, reuse by wrapping/padding.
    """
    if len(selected_list) >= need_each * 3:
        g1 = selected_list[0:need_each]
        g2 = selected_list[need_each:2*need_each]
        g3 = selected_list[2*need_each:3*need_each]
        return g1, g2, g3
    # Not enough unique -> reuse/rotate
    g1 = [selected_list[i % len(selected_list)] for i in range(0, need_each)]
    g2 = [selected_list[(i + need_each) % len(selected_list)] for i in range(0, need_each)]
    g3 = [selected_list[(i + 2*need_each) % len(selected_list)] for i in range(0, need_each)]
    return g1, g2, g3
...
def main(args):
    ...
    map_path = os.path.join(args.save_map_path, f'distraction_image_map_seed_{args.seed}_num_{args.num_images}.json')
    composite_root = os.path.join(args.distraction_image_save_path, f"{args.strategy_name}_subq3")
    ...
    if not os.path.isfile(map_path):
        raise RuntimeError(f"Distraction image map not found: {map_path}")
    ...
    art_font_path = "./Super Moods.ttf"  # 若不存在会自动 fallback
    ...
    for jb_file in jailbreak_files:
        ...
        for i, item in enumerate(tqdm(data, desc=f"{category}")):
            ...
            # 9 distractors source list
            selected_list = question2images.get(instruction, [])
            if not selected_list:
                print(f"[WARN] No distractors for: {instruction[:80]}...")
                continue

            g1, g2, g3 = split_24_for_three(selected_list, need_each=8)

            # Make text images for each sub-question
            os.makedirs("./combine_image_data", exist_ok=True)
            text_imgs = []
            text_img_paths = []
            for k in range(3):
                timg = text_to_art_image(sub_questions[k], font_path=art_font_path, font_size=50, text_color=(255, 0, 0))
                tmp_path = os.path.join("./combine_image_data", f"{category}_{i:05d}_art_text_image_{k+1}.jpg")
                timg.save(tmp_path, format="JPEG", quality=90)
                text_imgs.append(tmp_path)
                text_img_paths.append(tmp_path)

            # Build three 3x3 lists (8 distractors + 1 text at the end, index 9)
            # Group1
            tiles_1 = ensure_len(g1, 8, src_pool) + [text_img_paths[0]]
            tiles_2 = ensure_len(g2, 8, src_pool) + [text_img_paths[1]]
            tiles_3 = ensure_len(g3, 8, src_pool) + [text_img_paths[2]]

            # Compose three grids
            grid1 = make_grid_3x3(tiles_1, target_size=(500, 500))
            grid2 = make_grid_3x3(tiles_2, target_size=(500, 500))
            grid3 = make_grid_3x3(tiles_3, target_size=(500, 500))

            # Save composed images
            out1 = os.path.join(category_img_dir, f"{i}_1.jpg")
            out2 = os.path.join(category_img_dir, f"{i}_2.jpg")
            out3 = os.path.join(category_img_dir, f"{i}_3.jpg")
            grid1.save(out1, format="JPEG", quality=90)
            grid2.save(out2, format="JPEG", quality=90)
            grid3.save(out3, format="JPEG", quality=90)

            # One request, three images
            response_text = openai_vision_infer_multi(
                client=client,
                model=args.object_model,
                input_text=input_text,
                composed_imgs=[grid1, grid2, grid3],
                max_retries=5
            )

            # Append three records with the SAME response
            final_res.append({
                "prompt": input_text,
        ...
        # Save results
        out_dir = os.path.join(args.jailbreak_response_save_path, f"{args.strategy_name}_subq3", args.object_model)
